...
CREATE TABLE IF NOT EXISTS `sistema`.`users` (
  ...
  CONSTRAINT `users_role_id`
    FOREIGN KEY (`role_id`)
    REFERENCES `sistema`.`roles` (`id_roles`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `sistema`.`recoveries` (
  `id_passwors` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL,
  `token` VARCHAR(255) NOT NULL,
  `status` INT(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`id_passwors`, `user_id`),
